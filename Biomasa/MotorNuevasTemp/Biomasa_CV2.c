...
double Tht = 0;
//Parametros frio
double Tcold = 10; //temperatura de entrada (K)
...
int out(){
  ...
  //for (cont=0;cont<500;cont++){
    Tgn = Temperatura(Xg,T111,T222); //Temperatura actual del gas
    ...
    //te = Fg*tan(Fi_e)*(l-((pow(r,2))/(4*l))+(r*(cos(teta))+(r)/(4*l)*cos(2*teta)));
    //tc = Fg*tan(Fi_c)*(l-((pow(r,2))/(4*l))+r*(cos(teta-fi)+(r)/(4*l)*cos(2*teta-2*fi)));
    tf = W*b;
    ...
    if (pcs == 1){TG=Tgw;}
    else if (pcs == 2){TG=T222;}
    else if (pcs == 3){TG=Tgw;}
    else if (pcs == 4){TG=T111;} //que pasa si no es ninguna?
    Pg = (Mg * Rg * TG) / Vg;
    Vg = (Xc + Xe + Xwr) * Ap;
    ...
    //Txt
	  sprintf(text1,"%5.6f",te);
	  strcat(text1,"\t");
	  sprintf(text2,"%5.6f",tc);
	  strcat(text2,"\t");
	  sprintf(text3,"%5.6f",T111);
	  strcat(text3,"\t");
	  sprintf(text4,"%5.6f",T222);
	  strcat(text4,"\t");
	  sprintf(text5,"%5.6f",Vg);
	  strcat(text5,"\t");
	  sprintf(text6,"%5.6f",Pg);
	  strcat(text6,"\t");
	  sprintf(text7,"%5.6f",Xc);
	  strcat(text7,"\t");
	  sprintf(text8,"%5.6f",Xe);
	  strcat(text8,"\n");
