...
void setup()
{
  ...
  //Triangular a 5KHz -- TC6 (TC2 Channel 0) -- Salidas (4,5)
  ...
  REG_TC2_CMR0 = TC_CMR_BCPC_SET |                // Set TIOB on counter match with RC0
                 TC_CMR_ACPC_CLEAR |                  // Set TIOA on counter match with RC0
                 TC_CMR_BCPB_CLEAR |                // Clear TIOB on counter match with RB0
                 TC_CMR_ACPA_SET |                  // Clear TIOA on counter match with RA0
...
//Triangular a 5KHz -- TC7 (TC2 Channel 1) -- Salidas (3,10)
  REG_PMC_PCER1 |= PMC_PCER1_PID34;                 // Enable peripheral TC7 (TC2 Channel 1)
  REG_PIOC_ABSR |= PIO_ABSR_P29 | PIO_ABSR_P28;     // Switch the multiplexer to peripheral B for TIOA6 and TIOB6-- pag.859
  ...
  REG_TC2_CMR1 = TC_CMR_BCPC_SET |                // Set TIOB on counter match with RC1
                 TC_CMR_ACPC_CLEAR |                  // Set TIOA on counter match with RC1
                 TC_CMR_BCPB_CLEAR |                // Clear TIOB on counter match with RB1
                 TC_CMR_ACPA_SET |                  // Clear TIOA on counter match with RA1
                 TC_CMR_WAVE |                      // Enable wave mode
                 TC_CMR_WAVSEL_UP_RC |              // Count up with automatic trigger on RC compare
                 TC_CMR_EEVT_XC0 |                  // Set event selection to XC0 to make TIOB an output
                 TC_CMR_TCCLKS_TIMER_CLOCK1;        // Set the timer clock to TCLK1 (MCK/2 = 84MHz/2 = 48MHz)
  REG_TC2_RC1 = 8400;                               // Load the RC1 register  
  REG_TC2_CCR1 = TC_CCR_SWTRG | TC_CCR_CLKEN;       // Enable the timer TC7

  //Triangular a 5KHz -- TC0 (TC0 Channel 0) -- Salidas (2,13)
  REG_PMC_PCER0 |= PMC_PCER0_PID27;                 // Enable peripheral TC0 (TC0 Channel 0)--Buscar valor en "Peripheral Identifiers"
  REG_PIOB_ABSR |= PIO_ABSR_P25 | PIO_ABSR_P27;     // Switch the multiplexer to peripheral B for TIOA0 and TIOB0-- Buscar valor en "PIO Controller B Multiplexing"
  REG_PIOB_PDR |= PIO_PDR_P25 | PIO_PDR_P27;        // Disable the GPIO on the corresponding pins
  
  REG_TC0_CMR0 = TC_CMR_BCPC_SET |                // Clear TIOB on counter match with RC1         //CMR0 (channel 0)
                 TC_CMR_ACPC_CLEAR |                  // Set TIOA on counter match with RC1
                 TC_CMR_BCPB_CLEAR |                // Clear TIOB on counter match with RB1
                 TC_CMR_ACPA_SET |                  // Set TIOA on counter match with RA1
                 TC_CMR_WAVE |                      // Enable wave mode
                 TC_CMR_WAVSEL_UP_RC |              // Count up with automatic trigger on RC compare
                 TC_CMR_EEVT_XC0 |                  // Set event selection to XC0 to make TIOB an output
                 TC_CMR_TCCLKS_TIMER_CLOCK1;        // Set the timer clock to TCLK1 (MCK/2 = 84MHz/2 = 48MHz)
  REG_TC0_RC0 = 8400;                               // Load the RC1 register  
  REG_TC0_CCR0 = TC_CCR_SWTRG | TC_CCR_CLKEN;       // Enable the timer TC0


  //Para tiempo muerto "Non Overlapped Center Aligned Waveforms"
...
//  receiveRaspData();
  ...
  i1 = ((((int(1500)/1000.0)-1)));//v0001500w0001200z0001000 -- v000000w0000000z0001000 -- v0002000w0002000z0001000
  i2 = ((((int(1200)/1000.0)-1)));
  i3 = ((((int(v3)/1000.0)-1)));
  ...
  ma=(2800*(PrimeraABC(i1,i2,cont)+1.5))+10;
  mb=(2800*(SegundaABC(i1,i2,cont)+1.5))+10;
  mc=(2800*(TerceraABC(i1,i2,cont)+1.5))+10; 
...
//  REG_TC2_RA1 = mc;                              // Load the RA0 register
  REG_TC2_RA0 = ma;                              // Load the RA0 register
  REG_TC2_RB0 = ma;                              // Load the RB0 register
  REG_TC2_RA1 = mb;                              // Load the RA0 register
  REG_TC2_RB1 = mb;                              // Load the RA0 register
  REG_TC0_RA0 = mc;                              // Load the RB0 register
  REG_TC0_RB0 = mc;                              // Load the RA0 register
